...
bool Stack<ValueType>::Push(const ValueType& value) {
    _list.Insert(_list.Size(), value);
    // I need to return true or I need to return false depending on if it went through. idk how to do this.
    // It's a variable array list so it should always work right?
}
...
ValueType Stack<ValueType>::Pop() {
    ValueType tempVal;
    if (_list.Size() != 0) {
        _list.Remove(_list.Size() - 1, tempVal);
        return tempVal;
    } else {
        assert(false); // You called Pop() on an empty.
    }
}
...
ValueType Stack<ValueType>::Peek() {
    ValueType tempVal;
    if (_list.Size() != 0) {
        _list.Get(_list.Size() - 1, tempVal);
        return tempVal;
    } else {
        assert(false); // You called Peek() on an empty stack.
    }
}
